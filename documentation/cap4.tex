...
En las secciones siguientes se analizará como se ha integrado el simulador en Descubre y como es el modelo de programación que se usará para programar el simulador con iJava.


\subsection{Modelo de programación del simulador}
...
Por esto, es necesario insertar una instrucción que se ejecute ente las dos anteriores órdenes, y que detenga la cola de instrucciones de aceptar más instrucciones al robot.
...
El modelo se basará en la idea dejada por iJava, por supuesto, y se añadirá el estilo de programación de Arduino. En éste, se establece una función principal \texttt{setup()} que normalmente se usará para definir los parámetros del robot que se está programando y, después, se usará una función \texttt{loop()} que ejecutará en bucle las instrucciones que contenga. El uso de la función \texttt{loop()} es opcional, pudiendo meter todas las instrucciones en \texttt{setup()} y que este se ejecute una única vez.

Éste es el modelo que se seguirá.

La diferencia con iJava, es que no se debe usar la función \texttt{animate()} para indicar que cierta función se debe repetir en bucle. Ahora, se ejecutará la función \texttt{setup()} (que sustituirá a la función \texttt{main()} de iJava) y si la función \texttt{loop()} ha sido definida, se ejecutará en bucle automáticamente y de manera \emph{transparente} para el programador. 
...
Como se puede ver, esto producirá un bucle infinito que ejecutará la función \texttt{loop()}. 

...
El lenguaje Logo, {\color{red}desarrollado a finales de la decada de los 60} y basado en \Gls{lisp}, fue diseñado como una herramienta para aprendizaje. Todas sus características -interactividad, modularidad, extensibilidad, flexibilidad en los tipos de datos- persiguen esta meta. El lenguaje Logo fue uno de los primeros proyectos en emprender la difícil tarea de enseñar a programar a niños de Primaria.
...
\subsubsection*{Variables, definiendo nuevas operaciones y comentarios}
...
La orden \texttt{home} devuelve a la tortuga a la posición (0, 0) de la pantalla. Adicionalmente, se puede ocultar (o mostrar) la tortuga con las órdenes \texttt{showturtle} y \texttt{hideturtle}.
...
La instrucción \texttt{label} mostrará por pantalla el mensaje que se pase como argumento, pero este debe ser una \emph{word} o \emph{sentence}(solo un argumento)\footnote{Para más información sobre los tipos de datos \emph{words} y \emph{sentences} se puede consultar la sección \ref{sec:logo-variables}.}.

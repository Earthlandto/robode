...
//					if (node.yes.id !== "(block)") s = s + " ; ";
	...
	function operadorNew(node, context) {
		...
		if (node.datatype instanceof ArrayDatatype) {
			...
			if (node.defaultConstructor) {
				...
				s = s + ")";
			s = "(function(name, line) { runtime.docall(name, line); var res = " + s + "; runtime.doreturn(name, line); return res; })('Constructor de " + node.basetype + "', " + node.line + ")";
	 		}
		}
		return s;
	}
	...
	function call(node, context) {
		...
		if (!node.left.predefined) {
			s = "(function(name, line) { runtime.docall(name, line); var res = " + s + "; runtime.doreturn(name, line); return res; })('" + node.left.id + "', " + node.line + ")";
		}
		return s;
	}
	...
	function accesoNombrado(node, context) {
		...
		if (node.method) {
			s = "(function(name, line) { runtime.docall(name, line); var res = " + s + "; runtime.doreturn(name, line); return res; })('" + node.right.id + "', " + node.line + ")";
		}
		return s;
	}
